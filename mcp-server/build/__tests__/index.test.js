...
describe('Tyaprover MCP Server Tools', () => {
    ...
    describe('listApps', () => {
        it('should return apps list on successful API call', async () => {
            ...
            if (!tool)
                throw new Error('listApps tool not found');
            const result = await tool.handler({});
            expect(mockFetch).toHaveBeenCalledWith('http://fake-caprover.com/api/v2/user/apps', expect.objectContaining({ method: 'GET' }));
            expect(result.content[0].type).toBe('json_object');
            expect(result.content[0].json_object).toEqual([{ appName: 'app1' }, { appName: 'app2' }]);
        });
        it('should return error message on API failure', async () => {
            ...
            expect(result.content[0].text).toContain('Error listing apps: Tyaprover API request failed with status 500');
        });
    });
    describe('getAppDetails', () => {
        ...
        it('should return app details if app exists', async () => {
            ...
            if (!tool)
                throw new Error('getAppDetails tool not found');
            const result = await tool.handler({ appName });
            expect(result.content[0].type).toBe('json_object');
            expect(result.content[0].json_object).toEqual({ appName: 'app1', data: 'details1' });
        });
        it('should return error if app not found', async () => {
            ...
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe("Error: App 'nonexistentapp' not found.");
        });
    });
    describe('deployNewApp', () => {
        ...
        it('should call deploy API and return success message', async () => {
            ...
            if (!tool)
                throw new Error('deployNewApp tool not found');
            const result = await tool.handler(deployInput);
            expect(mockFetch).toHaveBeenCalledWith(`http://fake-caprover.com/api/v2/user/apps/appdefinitions/${deployInput.appName}`, expect.objectContaining({
                method: 'POST',
                body: JSON.stringify({
            ...
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Application '${deployInput.appName}' deployment/update initiated successfully.`);
        });
        it('should handle API error during deployment', async () => {
            ...
            const result = await tool.handler(deployInput);
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error deploying app '${deployInput.appName}': Tyaprover API request failed with status 400: Invalid app name`);
        });
    });
    describe('deleteApp', () => {
        ...
        it('should call delete API and return success message on successful deletion', async () => {
            ...
            if (!tool)
                throw new Error('deleteApp tool not found');
            const result = await tool.handler({ appName });
            expect(mockFetch).toHaveBeenCalledWith(`http://fake-caprover.com/api/v2/user/apps/appdefinitions/${appName}`, expect.objectContaining({ method: 'DELETE' }));
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Application '${appName}' deleted successfully. Description: App deleted successfully.`);
        });
        it('should return an error message if API call fails', async () => {
            ...
            const result = await tool.handler({ appName });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error deleting application '${appName}': Tyaprover API request failed with status 404: App not found on server`);
        });
        it('should handle unexpected non-BaseApi JSON error structure from API', async () => {
            ...
            expect(result.content[0].text).toContain(`Failed to delete application '${appName}'. Unexpected API response: ${JSON.stringify(mockErrorResponse)}`);
        });
        it('should handle error when callTyaproverApi itself throws (e.g. network issue)', async () => {
            ...
            const result = await tool.handler({ appName });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error deleting application '${appName}': Network connection refused`);
        });
    });
    describe('setAppEnvironmentVariables', () => {
        ...
        it('should fetch, update, and post app definition successfully', async () => {
            ...
            if (!tool)
                throw new Error('setAppEnvironmentVariables tool not found');
            const result = await tool.handler({ appName, environmentVariables: newEnvVars });
            expect(mockFetch).toHaveBeenCalledTimes(2);
            expect(mockFetch).toHaveBeenNthCalledWith(1, `http://fake-caprover.com/api/v2/user/apps`, expect.objectContaining({ method: 'GET' }));
            const expectedPayload = { ...initialAppDef, envVars: newEnvVars };
            expect(mockFetch).toHaveBeenNthCalledWith(2, `http://fake-caprover.com/api/v2/user/apps/appdefinitions/${appName}`, expect.objectContaining({
                method: 'POST',
                body: JSON.stringify(expectedPayload)
            }));
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Environment variables for '${appName}' updated successfully.`);
        });
        it('should return error if app not found during fetch', async () => {
            ...
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Error: Application '${appName}' not found.`);
        });
        it('should return error if fetch API call fails', async () => {
            ...
            const result = await tool.handler({ appName, environmentVariables: newEnvVars });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error setting environment variables for '${appName}': Tyaprover API request failed with status 500`);
        });
        it('should return error if update POST API call fails', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledTimes(2);
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Failed to update environment variables for '${appName}': Invalid update params`);
        });
    });
    describe('scaleApp', () => {
        ...
        it('should fetch, update instance count, and post app definition successfully', async () => {
            ...
            if (!tool)
                throw new Error('scaleApp tool not found');
            ...
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Application '${appName}' scaled to ${newInstanceCount} instance(s) successfully.`);
        });
        it('should return error if app not found during fetch', async () => {
            mockFetch.mockResolvedValueOnce({
                ok: true, status: 200, json: async () => ({
        ...
        it('should handle API error during POST for scaling', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledTimes(2);
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Failed to scale application '${appName}': Scaling failed internally`);
        });
    });
    describe('enableAppSsl', () => {
        ...
        it('should call enable SSL API and return success message', async () => {
            ...
            if (!tool)
                throw new Error('enableAppSsl tool not found');
            const result = await tool.handler({ appName, customDomain });
            expect(mockFetch).toHaveBeenCalledWith(`http://fake-caprover.com/api/v2/user/apps/appdefinitions/${appName}/customdomain`, expect.objectContaining({
                method: 'POST',
                body: JSON.stringify({ customDomain })
            }));
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`SSL enablement initiated for '${appName}' with domain '${customDomain}'. ${mockSuccessResponse.description}`);
        });
        it('should return an error message if API call fails (e.g., DNS not ready)', async () => {
            ...
            const result = await tool.handler({ appName, customDomain });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Failed to enable SSL for '${appName}' with domain '${customDomain}': ${mockErrorResponse.description}`);
        });
        it('should handle direct HTTP error from API call', async () => {
            ...
            const result = await tool.handler({ appName, customDomain });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error enabling SSL for '${appName}' with domain '${customDomain}': Tyaprover API request failed with status 500: Internal Server Error during SSL setup`);
        });
    });
    describe('removeCustomDomain', () => {
        ...
        it('should call remove custom domain API and return success message', async () => {
            ...
            if (!tool)
                throw new Error('removeCustomDomain tool not found');
            const result = await tool.handler({ appName, customDomain });
            expect(mockFetch).toHaveBeenCalledWith(`http://fake-caprover.com/api/v2/user/apps/appdefinitions/${appName}/customdomain`, expect.objectContaining({
                method: 'DELETE',
                body: JSON.stringify({ customDomain })
            ...
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toBe(`Custom domain '${customDomain}' removed from '${appName}'. ${mockSuccessResponse.description}`);
        });
        it('should return an error message if API call fails (e.g., domain not found on app)', async () => {
            ...
            const result = await tool.handler({ appName, customDomain });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Failed to remove custom domain '${customDomain}' from '${appName}': ${mockErrorResponse.description}`);
        });
        it('should handle direct HTTP error from API call for remove domain', async () => {
            ...
            const result = await tool.handler({ appName, customDomain });
            expect(result.content[0].type).toBe('text');
            expect(result.content[0].text).toContain(`Error removing custom domain '${customDomain}' from '${appName}': Tyaprover API request failed with status 404: App not found`);
        });
    });
});
export {};
